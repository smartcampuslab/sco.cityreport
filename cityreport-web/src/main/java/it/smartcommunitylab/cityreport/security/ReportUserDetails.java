...
public class ReportUserDetails implements UserDetails {
	 ...
	 * @param username
	 */
	public ReportUserDetails(String username) {
		this.user = username;
	}

	@Override
	public Collection<? extends GrantedAuthority> getAuthorities() {
		return Collections.unmodifiableCollection(REPORTER_AUTHORITIES);
	}
	 ...
	 * @return
	 */
	public static boolean isReporter(Authentication authentication) {
		if (authentication.getAuthorities() == null || authentication.getAuthorities().isEmpty()) return false;
		for (GrantedAuthority a : authentication.getAuthorities()) {
			if (a.getAuthority().equals(REPORTER)) return true;
		}
		return false;
	}
